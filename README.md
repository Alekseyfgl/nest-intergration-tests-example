...
В проекте настроены различные способы запуска тестов. Ниже представлены доступные команды и их описание.
...
### Тесты по типам и директориям

```bash
# Запуск тестов из указанной директории (указывается после --)
npm run test:path -- src/utils

# Запуск тестов для модуля utils
npm run test:utils
...
# Запуск всех тестов по различным шаблонам
npm run test:all

# Запуск тестов из директории test по конфигурации jest-all.json
npm run test:dir
```
...
# Тесты запускаются асинхронно с использованием 4 параллельных процессов
...
npm run test:with-docker --path="(src/utils|src/services)"
```
...
Для обеспечения изоляции при параллельном запуске тестов:

```bash
# Запуск тестов сразу нескольких сервисов
npm run test:with-docker --path="(src/users|src/orders)"

# Асинхронный запуск с указанным количеством параллельных потоков
npm run test:with-docker --path=src --maxWorkers=4

# Увеличение количества воркеров для мощных машин
npm run test:with-docker --path=src --maxWorkers=8

# Уменьшение количества воркеров для слабых машин
npm run test:with-docker --path=src --maxWorkers=2
```

Если не указать `--maxWorkers`, будет использовано значение по умолчанию (4).
